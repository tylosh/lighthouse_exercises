...
function min(numbers) {
  var result = numbers[0]; //use the first number in array as the initial comparison
  for (var i = 0; i <= numbers.length; i++) { //go through loop of the array length
    if (numbers[i] <= result) { //if the number in the array in current loop is lower than the current result, update result
      result = numbers[i]
    }
  }
  return result
